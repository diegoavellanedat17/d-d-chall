...
export class DiagramWidget extends React.Component {
      ...
      action.selectionModels.forEach(model => {
        if (model.model instanceof NodeModel || model.model instanceof PointModel) {
          ...
          if(model.model instanceof PointModel){
            ...
            let snapX = model.initialX + ((event.pageX - this.state.action.mouseX) / (diagramModel.getZoomLevel() / 100));
            let snapY = model.initialY + ((event.pageY - this.state.action.mouseY) / (diagramModel.getZoomLevel() / 100));
            [model.model.x,model.model.y]= gridSnap(snapX ,snapY)
          }
  ...
  onMouseUp(event) {
    ...
    if (element === null) {
      ...
      //console.log(element.model.link)
      ...
      action.selectionModels.forEach(model => {
        ...
        if (model.model instanceof PointModel) {
          ...
          if (element.model instanceof PortModel) {
            ...
            //console.log(model.model.getLink())
